...
pub fn assign_bend_points(graph: &mut Graph) {
    ...
    for edge in &mut edges {
        ...
        let (from_x, from_y) = (
            from_node.x.unwrap(),
            from_node.y.unwrap() + from_node.y_offset.unwrap_or(0.0),
        );
        let (to_x, to_y) = (
            to_node.x.unwrap(),
            to_node.y.unwrap() + to_node.y_offset.unwrap_or(0.0),
        );
        ...
        let (edge_from_x, edge_from_y) =
            if to_center_x > from_center_x && to_center_y > from_center_y {
        ...
        edge.adjusted_points = Some(
            vec![(edge_from_x, edge_from_y)]
                .into_iter()
